...
class Maze :
    ...
    def reset( self ):
        for i in range(self._mazeCells.num_rows()):
            for j in range(self._mazeCells.num_cols()):
                if self._mazeCells[i][j] == Maze.PATH_TOKEN or self._mazeCells[i][j] == Maze.TRIED_TOKEN:
                    self._mazeCells[i][j] = Maze.EMPTY_TOKEN
        return self._mazeCells

    # Prints a text-based representation of the maze.
    def draw( self ):
        for i in self._mazeCells:
            print(" ".join(i))
